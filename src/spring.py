...
class DemoRope(App):
    ...
    oscillation_amplitude_x = 75  # Small amplitude for x-axis
    ...
    def StopGraphRecording(self):
        ...
        # График смещения
        for i, displacements in self.y_displacements.items():
            axs[0].plot(displacements, label=f"Particle {i}")
        axs[0].set_xlabel("Time")
        axs[0].set_ylabel("Y Displacement from Initial Position")
        axs[0].legend()
        axs[0].set_title("Y Displacement Over Time")
        """
        # График скорости
